classdef rangeImage < handle
    ...
    methods(Access = public)
        function obj = rangeImage(ranges,skip,cleanFlag, pose1)
            ...
            % Constructs a raeImage for the supplied data.
...
%                     obj.walledge2 = pass;
%                 end
%                 obj.walledge1=floor(obj.walledge1/skip);
%                 obj.walledge2=floor(obj.walledge2/skip);
                n=0;
        ...
        function modelPts = getmodelPts(obj)
            ...
            %             number= begin-End;
             %           a=length(index);
               wpts1 = obj.pose.aToB() *[0;1.8288;1];
                wpts2 = obj.pose.aToB() *[1.8288;0;1];
...
%                 obj.walledge2 = floor(wtheta2*180/pi);
                obj.walledge1 = floor(wtheta1*180/pi/obj.skip);
                obj.walledge2 = floor(wtheta2*180/pi/obj.skip);      
            modelPts=zeros(3,15);
           % thetaa =  atan2(sin(obj.pose.th), cos(obj.pose.th));
            
            if obj.walledge1>obj.walledge2
                a= obj.numPix-obj.walledge1 + obj.walledge2;
            else
                a= obj.walledge2 - obj.walledge1;
            end
...
%                 start=obj.walledge1;
            ...
            for i=1:15
                ...
                %                        continue;
                %                    else
                start=obj.increase(start,obj.numPix,scale);
                modelPts(1,i)=obj.xArray(start);
                modelPts(2,i)=obj.yArray(start);
                modelPts(3,i)=1;
...
%                                      modelPts(3,i)=1;
            ...
            a(a==Inf)=[];
            a(a==-Inf)=[];
            if isempty(modelPts)||length(a)<4
                obj.walsucflag=0;
                return;
            end
        end
 ...
 function findObject(obj)
            m=0;
            error= 0.03;%leave a little space for error
            obj.rArray(obj.rArray==0)=Inf;
            obj.rArray(obj.rArray>1.8)=Inf; % limit the distance
            obj.rArray(1,18:342)=Inf;
            ...
            if(m==0)
                display('there is no element in array!');
                obj.objsucflag=0;
                return;
            end
            ...
            for i = 1:m
                if i+1 <=m
                    if  abs(obj.objcell{1,ansr}(2,1))<abs(obj.objcell{1,i+1}(2,1))
                        ansr = i;
                    else
                        if obj.objcell{1,i+1}(1,1)-obj.objcell{1,ansr}(1,1)<0.2
                           ansr=i+1;
            ...
            if isempty(obj.newxArray)
                display('newxArray is empty!');
                obj.objsucflag=0;
                return;
            end
            if isempty(obj.newyArray)
                display('newyArray is empty!');
                obj.objsucflag=0;
                return;
            end
            ...
            if line(1)==Inf || line(1)==-Inf ||line(2)==Inf || line(2)==-Inf % the condition of x=constant
                ...
                obj.middley = (range1y+range2y)/2;
                 ...
                 checkx=diff(obj.newxArray);
            %checky=diff(obj.newxArray);
                 [~,inx]=find(abs(checkx)>0.01);
                 xA=obj.newxArray;
                 yA=obj.newyArray;
                 xA(inx+1)=[];
                 yA(inx+1)=[];
                 
                an1x=line(1)*(yA(1)-line(2)+1/line(1)*xA(1))/(1+line(1)^2);
                an1y=line(1)*(xA(1)+line(2)/line(1)+line(1)*yA(1))/(1+line(1)^2);
                an2x=line(1)*(yA(end)-line(2)+1/line(1)*xA(end))/(1+line(1)^2);
                an2y=line(1)*(xA(end)+line(2)/line(1)+line(1)*yA(end))/(1+line(1)^2);
...
%                 obj.theta=atan2(-range2x+range1x,range2y-range1y);
                obj.theta=atan2(an2y-an1y,an2x-an1x);
                if obj.theta>0
                    obj.theta=atan2(an1y-an2y,an1x-an2x);
                end
            ...
            display(obj.middley);
            display(obj.theta);
%             if abs(obj.theta)>0.5
%                 obj.theta=0.2;
%             end
%             if(obj.theta>pi/2-0.3||obj.thta<pi/2+0.3)
%                 obj.theta=obj.theta-pi/2;
%             end
        ...
        function out = increase(obj,in,num,scaler)
            % increment with wraparound over natural numbers
            out = indexAdd(obj,in,num,scaler);
        end
